import { ConfigModule } from '@nestjs/config';
import { Test, TestingModule } from '@nestjs/testing';
import { UnauthorizedException } from '@nestjs/common';

jest.mock('src/generated/prisma/client', () => ({
  Prisma: {
    PrismaClientKnownRequestError: class PrismaClientKnownRequestError extends Error {
      code?: string;
    },
  },
  PrismaClient: class PrismaClient {},
}));

import { AuthController } from '../auth/auth.controller';
import { AuthModule } from '../auth/auth.module';
import { ChangePassValidationPipe } from '../auth/pipes/change-password.pipe';
import { AuthPasswordService } from '../auth/services/auth-password.service';
import { AuthTokenService } from '../auth/services/auth-token.service';
import { OAuthGoogleService } from '../auth/services/oauth.service';
import appConfig from '../config/app.config';
import authConfig from '../config/auth.config';
import passwordConfig from '../config/password.config';
import { PrismaService } from '../prisma/prisma.service';

describe('Password Management Requirement 2.3', () => {
  let controller: AuthController;
  let changePasswordValidationPipe: ChangePassValidationPipe;
  let passwordService: AuthPasswordService;
  let tokenService: AuthTokenService;

  const prismaMock = {
    user: {
      findUnique: jest.fn(),
      update: jest.fn(),
    },
    jWTToken: {
      updateMany: jest.fn(),
    },
  };

  const oauthService = {
    createAuthRedirectUrl: jest.fn(),
    handleCallback: jest.fn(),
  };

  beforeAll(async () => {
    process.env.JWT_PRIVATE_KEY =
      '-----BEGIN PRIVATE KEY-----\\nkyJv0k0wkAnhkAUglOAjHAhC\n-----END PRIVATE KEY-----';
    process.env.JWT_PUBLIC_KEY =
      '-----BEGIN PUBLIC KEY-----\\nzQIDAQAB\n-----END PUBLIC KEY-----';
    process.env.JWT_DURATION = '900';
    process.env.JWT_REFRESH_DURATION = '604800';
    process.env.PASSWORD_REGEX =
      '^(?=.*[A-Z])(?=.*\\d)(?=.*[^A-Za-z0-9]).{10,}$';
    process.env.PASSWORD_ERROR_MESSAGE =
      'Password must be at least 10 characters and include one uppercase letter, one number, and one symbol.';

    const moduleFixture: TestingModule = await Test.createTestingModule({
      imports: [
        ConfigModule.forRoot({
          isGlobal: true,
          cache: true,
          load: [appConfig, authConfig, passwordConfig],
        }),
        AuthModule,
      ],
    })
      .overrideProvider(PrismaService)
      .useValue(prismaMock)
      .overrideProvider(OAuthGoogleService)
      .useValue(oauthService)
      .compile();

    controller = moduleFixture.get(AuthController);
    changePasswordValidationPipe = moduleFixture.get(ChangePassValidationPipe);
    passwordService = moduleFixture.get(AuthPasswordService);
    tokenService = moduleFixture.get(AuthTokenService);
  });

  beforeEach(() => {
    jest.clearAllMocks();
  });

  it('changes the password and revokes all active tokens', async () => {
    const currentHash = await passwordService.hash('SecurePass1_');
    prismaMock.user.findUnique.mockResolvedValue({
      id: 'user-1',
      username: 'diego_123',
      email: 'diego@example.com',
      passwordHash: currentHash,
    });
    prismaMock.user.update.mockImplementation(async ({ data }) => ({
      id: 'user-1',
      username: 'diego_123',
      email: 'diego@example.com',
      passwordHash: data.passwordHash,
    }));
    prismaMock.jWTToken.updateMany.mockResolvedValue({ count: 4 });

    const token = tokenService.signAccess({
      sub: 'user-1',
      type: 'access',
      jti: 'token-1',
      exp: Date.now() + 1000 * 60 * 15,
    });
    const payload = changePasswordValidationPipe.transform({
      currentPassword: 'SecurePass1_',
      newPassword: 'SecurePass2_',
    });
    const response = {
      clearCookie: jest.fn(),
    } as any;

    await controller.changePassword(token, response, payload);

    expect(prismaMock.user.update).toHaveBeenCalledTimes(1);
    expect(prismaMock.user.update).toHaveBeenCalledWith({
      where: { username: 'diego_123' },
      data: {
        email: 'diego@example.com',
        passwordHash: expect.any(String),
        username: 'diego_123',
      },
    });
    expect(prismaMock.user.update.mock.calls[0][0].data.passwordHash).not.toBe(
      'SecurePass2_',
    );
    expect(prismaMock.user.update.mock.calls[0][0].data.passwordHash).toContain(
      ':',
    );
    expect(prismaMock.jWTToken.updateMany).toHaveBeenCalledWith({
      where: { userId: 'user-1' },
      data: { revoked: true },
    });
    expect(response.clearCookie).toHaveBeenCalledWith('access_token', {
      httpOnly: true,
      secure: false,
      sameSite: 'strict',
      path: '/',
    });
  });

  it('rejects password change when current password is invalid', async () => {
    const currentHash = await passwordService.hash('SecurePass1_');
    prismaMock.user.findUnique.mockResolvedValue({
      id: 'user-1',
      username: 'diego_123',
      email: 'diego@example.com',
      passwordHash: currentHash,
    });

    const token = tokenService.signAccess({
      sub: 'user-1',
      type: 'access',
      jti: 'token-2',
      exp: Date.now() + 1000 * 60 * 15,
    });
    const payload = changePasswordValidationPipe.transform({
      currentPassword: 'WrongPass1_',
      newPassword: 'SecurePass2_',
    });

    await expect(
      controller.changePassword(
        token,
        {
          clearCookie: jest.fn(),
        } as any,
        payload,
      ),
    ).rejects.toThrow(new UnauthorizedException('Invalid credentials'));

    expect(prismaMock.user.update).not.toHaveBeenCalled();
    expect(prismaMock.jWTToken.updateMany).not.toHaveBeenCalled();
  });
});
