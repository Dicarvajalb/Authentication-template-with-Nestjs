import { ConfigModule } from '@nestjs/config';
import { Test, TestingModule } from '@nestjs/testing';
import { UnauthorizedException } from '@nestjs/common';

jest.mock('src/generated/prisma/client', () => ({
  Prisma: {
    PrismaClientKnownRequestError: class PrismaClientKnownRequestError extends Error {
      code?: string;
    },
  },
  PrismaClient: class PrismaClient {},
}));

import { AuthController } from '../auth/auth.controller';
import { AuthModule } from '../auth/auth.module';
import { LoginValidationPipe } from '../auth/pipes/login.pipe';
import { AuthPasswordService } from '../auth/services/auth-password.service';
import { OAuthGoogleService } from '../auth/services/oauth.service';
import appConfig from '../config/app.config';
import authConfig from '../config/auth.config';
import passwordConfig from '../config/password.config';
import { PrismaService } from '../prisma/prisma.service';

describe('Login Requirement 2.2', () => {
  let controller: AuthController;
  let loginValidationPipe: LoginValidationPipe;
  let passwordService: AuthPasswordService;

  const prismaMock = {
    user: {
      findFirst: jest.fn(),
    },
    loginAttempt: {
      findUnique: jest.fn(),
      upsert: jest.fn(),
    },
    jWTToken: {
      create: jest.fn(),
    },
  };

  const oauthService = {
    createAuthRedirectUrl: jest.fn(),
    handleCallback: jest.fn(),
  };

  beforeAll(async () => {
    process.env.JWT_PRIVATE_KEY =
      '-----BEGIN PRIVATE KEY-----\\nkyJv0k0wkAnhkAUglOAjHAhC\n-----END PRIVATE KEY-----';
    process.env.JWT_PUBLIC_KEY =
      '-----BEGIN PUBLIC KEY-----\\nzQIDAQAB\n-----END PUBLIC KEY-----';
    process.env.JWT_DURATION = '900';
    process.env.JWT_REFRESH_DURATION = '604800';
    process.env.PASSWORD_REGEX =
      '^(?=.*[A-Z])(?=.*\\d)(?=.*[^A-Za-z0-9]).{10,}$';
    process.env.PASSWORD_ERROR_MESSAGE =
      'Password must be at least 10 characters and include one uppercase letter, one number, and one symbol.';

    const moduleFixture: TestingModule = await Test.createTestingModule({
      imports: [
        ConfigModule.forRoot({
          isGlobal: true,
          cache: true,
          load: [appConfig, authConfig, passwordConfig],
        }),
        AuthModule,
      ],
    })
      .overrideProvider(PrismaService)
      .useValue(prismaMock)
      .overrideProvider(OAuthGoogleService)
      .useValue(oauthService)
      .compile();

    controller = moduleFixture.get(AuthController);
    loginValidationPipe = moduleFixture.get(LoginValidationPipe);
    passwordService = moduleFixture.get(AuthPasswordService);
  });

  beforeEach(() => {
    jest.clearAllMocks();
  });

  it('allows multiple successful login requests with the same valid credentials', async () => {
    const hashedPassword = await passwordService.hash('StrongPass1!');
    prismaMock.user.findFirst.mockResolvedValue({
      id: 'user-1',
      email: 'diego@example.com',
      username: 'diego_123',
      passwordHash: hashedPassword,
    });
    prismaMock.loginAttempt.findUnique.mockResolvedValue({
      failedCount: 2,
      lockedUntil: null,
    });
    prismaMock.loginAttempt.upsert.mockResolvedValue(undefined);
    prismaMock.jWTToken.create.mockResolvedValue(undefined);

    const payload = loginValidationPipe.transform({
      email: 'diego@example.com',
      password: 'StrongPass1!',
    });

    const firstResponse = await controller.signIn(payload, {
      cookie: jest.fn(),
    } as any);
    const secondResponse = await controller.signIn(payload, {
      cookie: jest.fn(),
    } as any);

    expect(firstResponse.access_token).toEqual(expect.any(String));
    expect(firstResponse.refresh_token).toEqual(expect.any(String));
    expect(secondResponse.access_token).toEqual(expect.any(String));
    expect(secondResponse.refresh_token).toEqual(expect.any(String));
    expect(secondResponse.access_token).not.toEqual(firstResponse.access_token);
    expect(secondResponse.refresh_token).not.toEqual(
      firstResponse.refresh_token,
    );
    expect(prismaMock.user.findFirst).toHaveBeenCalledTimes(2);
    expect(prismaMock.loginAttempt.upsert).toHaveBeenCalledTimes(2);
    expect(prismaMock.loginAttempt.upsert).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({
        where: { userId: 'user-1' },
        create: {
          userId: 'user-1',
          failedCount: 0,
          lockedUntil: null,
        },
        update: expect.objectContaining({
          failedCount: 0,
          lockedUntil: null,
        }),
      }),
    );
    expect(prismaMock.jWTToken.create).toHaveBeenCalledTimes(4);
  });

  it('rejects invalid credentials and increments failed login attempts', async () => {
    const hashedPassword = await passwordService.hash('StrongPass1!');
    prismaMock.user.findFirst.mockResolvedValue({
      id: 'user-1',
      email: 'diego@example.com',
      username: 'diego_123',
      passwordHash: hashedPassword,
    });
    prismaMock.loginAttempt.findUnique.mockResolvedValue({
      failedCount: 1,
      lockedUntil: null,
    });
    prismaMock.loginAttempt.upsert.mockResolvedValue(undefined);

    const payload = loginValidationPipe.transform({
      email: 'diego@example.com',
      password: 'WrongPass1!',
    });

    await expect(
      controller.signIn(payload, {
        cookie: jest.fn(),
      } as any),
    ).rejects.toThrow(new UnauthorizedException('Invalid credentials'));

    expect(prismaMock.user.findFirst).toHaveBeenCalledTimes(1);
    expect(prismaMock.loginAttempt.upsert).toHaveBeenCalledWith({
      where: { userId: 'user-1' },
      create: {
        userId: 'user-1',
        failedCount: 2,
        lockedUntil: null,
      },
      update: expect.objectContaining({
        failedCount: 2,
        lockedUntil: null,
      }),
    });
    expect(prismaMock.jWTToken.create).not.toHaveBeenCalled();
  });
});
